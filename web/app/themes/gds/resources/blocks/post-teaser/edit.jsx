...
function BlockEdit(props) {
  ...
  const { teaser } = useSelect((select) => {
    ...
    const hasResolved = hasFinishedResolution('getEntityRecord', ['postType', postType, postId])
    return {
      ...
      { showPlaceholder ? (
        <div { ...blockProps }>
          <PostPlaceholder
            postType={ postType }
            postId={ postId }
            onChange={ (post) => setAttributes({
              postId: post.id,
              postType: post.subtype}
            ) }
            onSetPostType={ (newPostType) => setAttributes({
