...
/*
Plugin Name:  Gutenberg Global Styles
Plugin URI:   https://genero.fi
Description:  Gutenberg Global Styles split out from Gutenberg plugin
Version:      1.0.0
Author:       Genero
...
/**
 * @see https://github.com/WordPress/gutenberg/blob/v9.2.1/lib/global-styles.php
 */
...
add_action('plugins_loaded', function () {
    // Skip if Gutenberg plugin is enabled since it provides this as of v8.9.x
    if (in_array('gutenberg/gutenberg.php', apply_filters('active_plugins', get_option('active_plugins')))) {
        return;
    }
    // Skip if WP already comes bundled with global styles (v5.6.0+)
    if (version_compare(get_bloginfo('version'), '5.6.0', '>=')) {
        return;
    }

    // Initialize and add the global styles css variables to the page.
    add_action('wp_enqueue_scripts', __NAMESPACE__ . '\\gutenberg_experimental_global_styles_enqueue_assets');
    add_action('enqueue_block_editor_assets', __NAMESPACE__ . '\\gutenberg_experimental_global_styles_enqueue_assets');
});
